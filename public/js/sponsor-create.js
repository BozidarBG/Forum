...
$('#save').on('click', function(){
    ...
    if(name=="" || position =="" || link=="" || banner==null){
        //display errors
        if(name==""){
        ...
        if(!banner){
            $('#file-upload').prev().prev().text('Banner -> This field can\'t be empty').css('color', 'red');
        }
        return;
    }
    ...
    $.ajax({
        ...
        success: function(data) {
            if (data == "success") {
                toastr.success('Sponsor saved successfully!');
                setTimeout(function(){
                    location.reload();
                }, 1500);
            } else {
